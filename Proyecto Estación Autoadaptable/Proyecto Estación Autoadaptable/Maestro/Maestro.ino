// Solucion taller estacion - codigo maestro
...
void MEF_Sistema() {
  switch (eSistema) {
    case EOK:
      digitalWrite(LG,HIGH);
      digitalWrite(COOLER, LOW);
      Serial.println("[MEF]\tCOOLER APAGADO");
      P = 0;
      readDHT();
      POST();
      if (T > Tmax) {
    ...
    case EWARN:
      ...
      P = 1;
      digitalWrite(COOLER, HIGH);
      Serial.println("[MEF]\tCOOLER ENCENDIDO");
      readDHT();
      POST();
      if (T < Tmin) {
        Serial.println("[MEF]\tSistema normal");
        eSistema = EOK;
        C = 0;
        te = millis();
      }else if ((millis()-te > e) && (C<3)){
